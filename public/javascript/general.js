$(document).ready(function($) {	
	...
	*/	
	$("#DepartureForm").on("submit", function (e) {
		...
		// Get airline codes
		var airline_codes = $('#departure_airlines').val().split(',');

		var promises = [];
		InitializeModalPlaceholder("#DeparturesModalPlaceholder");
		var table = InitializeDeparturesTable();
		...
		$.when.apply(null, promises).done(function(){
			$('#DeparturesLoading').empty();
			$('#DeparturesTablePlaceholder').html(table);
			ResetAirlinesAndAirportsFields();
		})
	...
	*/	
	$("#ReturnForm").on("submit",function  (e) {
		...
		// Get airline codes
		var airline_codes = $('#return_airlines').val().split(',');

		var promises = [];
		InitializeModalPlaceholder("#ReturnsModalPlaceholder");
		...
		for (var i = 0; i < airline_codes.length; i++) {
			var request = $.post("/search/" + airline_codes[i], $(this).serialize(), function( data ) {
				var flights = data;
				CreateTable(table, flights);
				CreateModal(flights, "ReturnsModal", "#ReturnsModalPlaceholder");
			});
			promises.push(request);
		}
		...
		// When all requests are completed display the table and remove loader animation.
		$.when.apply(null, promises).done(function(){
			$('#ReturnsLoading').empty();
			$('#ReturnsTablePlaceholder').html(table);
			ResetAirlinesAndAirportsFields();
		})
	...
	function FormInputVerification (e, FromId, ToId, dateId, LoaderId, TablePlaceHolderId, errorId,
		...
		//Check if filled date is later than the current date and set error if false.
			...
			if (!from_airport_code || !to_airport_code) {
				$(LoaderId).empty();
				$(TablePlaceHolderId).empty();
				$(errorId).html("Please select the departure and arrival city with airport from values provided by autocomplete").addClass("ReturnsError").show();
				setTimeout(function() {
					$(errorId).fadeOut().empty();
				}, 2000);
				return 1;
	...
	*/
	function ResetAirlinesAndAirportsFields () {
